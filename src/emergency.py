...
LIMITATION: This uses simple substring/keyword matching, not clinical
NLP or negation handling. E.g. "no chest pain" will still match
"chest pain" as a false positive — that's intentional, since a false
alarm is far cheaper than a missed emergency. This is a best-effort
safety net, not a diagnostic system.
"""
...
def check_emergency(message: str) -> bool:
    """
    Check whether a message contains high-risk emergency symptoms.

    Args:
        message: Raw or translated user message.

    Returns:
        True if an emergency phrase or term combination is detected,
        otherwise False.
    """
    if not message:
        return False
...
def get_emergency_response() -> str:
    """
    Build the emergency alert message shown to the user.

    Returns:
        A formatted alert string directing the user to call 1122.
    """
    return (
        "⚠️ Emergency symptoms detected.\n\n"
        "Your symptoms may require immediate medical attention. "
        "Please call Rescue 1122 or go to the nearest emergency "
